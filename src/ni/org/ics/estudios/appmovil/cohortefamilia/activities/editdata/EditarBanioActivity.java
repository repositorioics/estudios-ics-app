...
import ni.org.ics.estudios.appmovil.cohortefamilia.activities.ListaBaniosActivity;
import ni.org.ics.estudios.appmovil.cohortefamilia.forms.AreaAmbienteCasaFormLabels;
import ni.org.ics.estudios.appmovil.cohortefamilia.forms.BanioForm;
...
public class EditarBanioActivity extends FragmentActivity implements
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		username =
				settings.getString(PreferencesActivity.KEY_USERNAME,
						null);
		infoMovil = new DeviceInfo(EditarBanioActivity.this);
        ...
        mWizardModel = new BanioForm(this,mPass);
    ...
    public void saveData(){
        try {
            ...
            //Obtener datos del bundle para la banio
            ...
            if (tieneValor(largo)) {
                banio.setLargo(Double.parseDouble(largo));
            } else {
                banio.setLargo(null);
            }
            if (tieneValor(ancho)) {
                banio.setAncho(Double.parseDouble(ancho));
            } else {
                banio.setAncho(null);
            }
            Page area = mWizardModel.findByKey(labels.getTotalM2());
            if (tieneValor(area.getHint())) {
                banio.setTotalM2(Double.valueOf(area.getHint()));
            }
            if (tieneValor(conVentana)) {
                ...
                banio.setConVentana(catConVentana.getCatKey());
            }
            banio.setRecordDate(new Date());
            banio.setRecordUser(username);
            banio.setDeviceid(infoMovil.getDeviceId());
            banio.setEstado('0');
            banio.setPasive('0');
