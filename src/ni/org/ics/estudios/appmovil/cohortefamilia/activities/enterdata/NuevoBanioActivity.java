...
public class NuevoBanioActivity extends FragmentActivity implements
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		username =
				settings.getString(PreferencesActivity.KEY_USERNAME,
						null);
		infoMovil = new DeviceInfo(NuevoBanioActivity.this);
    ...
    public void saveData(){
		...
		Banio b = new Banio();
		b.setCodigo(id);
		b.setCasa(areaCasa.getCasa());
		b.setAreaAmbiente(areaCasa);
		
		if (tieneValor(largo)) {
			b.setLargo(Double.valueOf(largo));
		}
		if (tieneValor(ancho)) {
			b.setAncho(Double.valueOf(ancho));
		}
		Page area = mWizardModel.findByKey(labels.getTotalM2());
		if (tieneValor(area.getHint())) {
			b.setTotalM2(Double.valueOf(area.getHint()));
		}
		if (tieneValor(conVentana)) {
			...
			b.setConVentana(catConVentana.getCatKey());
		}
		b.setTipo("banio");
