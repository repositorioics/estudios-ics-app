...
public class CandidatoTransmisionCovid19Adapter extends ArrayAdapter<CandidatoTransmisionCovid19> {

	private SimpleDateFormat mDateFormat = new SimpleDateFormat("MMM dd, yyyy");
    private SimpleDateFormat formatter = new SimpleDateFormat("dd/MM/yyyy");
	private List<MessageResource> catalogos;
	public CandidatoTransmisionCovid19Adapter(Context context, int textViewResourceId,
                                              List<CandidatoTransmisionCovid19> items, List<MessageResource> catalogos) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		CandidatoTransmisionCovid19 p = getItem(position);
		if (p != null && p.getParticipante()!=null) {

			TextView textView = (TextView) v.findViewById(R.id.identifier_text);
			textView.setTextColor(Color.BLACK);
			if (textView != null) {
				textView.setText(this.getContext().getString(R.string.code) + ": " + p.getParticipante().getCodigo()+ " - Casa CHF: " + p.getCasaCHF());
			}
