...
public class ListaMuestrasVacunasUO1Activity extends AbstractAsyncListActivity {
	...
	private static VisitaVacunaUO1 visitaVacunaUO1 = new VisitaVacunaUO1();
	//Tipo de lista
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        visitaVacunaUO1 = (VisitaVacunaUO1) getIntent().getExtras().getSerializable(Constants.VISITA);
        if (visitaVacunaUO1 !=null) {
            participante = visitaVacunaUO1.getParticipante();
            fechaVisita = visitaVacunaUO1.getFechaVisita();
        }
	...
	private void createDialogMuestras(final String opcion) {
		...
		int edadMeses = participante.getEdadMeses();
		/*Volumen de muestra para Serologia UO1 (Aguda y Convaleciente)
		 * < de 6 meses	| >= 6 meses y < de 2 años	| >= 2 años
		 *      N/A 	|           2 ml	        |   2 ml
		 */
		if (opcion.equalsIgnoreCase(Constants.CODIGO_TUBO_ROJO) && !participante.getEdad().equalsIgnoreCase("ND")) {
			//>= 2 años
			if (edadMeses >= 24) {
		 ...
		 *      2 ml 	|           2 ml	        |   6 ml
		 */
		if (opcion.equalsIgnoreCase(Constants.CODIGO_TUBO_PBMC) && !participante.getEdad().equalsIgnoreCase("ND")) {
			//>= 2 años
			if (edadMeses >= 24) {
				...
				labelMuestra = getString(R.string.pbmc2mlVacunaUO1_1);
				labelVolumenPermitido = "";
		...
		if (volumenTotalPermitido > volumenExistente) {
			...
			builder.setMessage(labelMuestra + "\n" + getString(R.string.code) + ": " + participante.getCodigo() + " - " + participante.getNombre1() + " " + participante.getApellido1());
	...
	private class FetchDataMuestrasCasosTask extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... values) {
			try {
				...
				mMuestras = estudiosAdapter.getMuestras(MuestrasDBConstants.participante + " = " + participante.getCodigo()
		...
		protected void onPostExecute(String resultado) {
			...
			textView.setText(getString(R.string.main_3) + "\n" + getString(R.string.follow_up_list_samp) + "\n" +
                        getString(R.string.code) + ": " + participante.getCodigo() + " - " +
					(visitaVacunaUO1.getVisita().equalsIgnoreCase("I")?getString(R.string.visita_inicial):getString(R.string.visita_final)));
	...
	private class OpenDataEnterActivityTask extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... values) {
			try {
				opcion = values[0];
				Bundle arguments = new Bundle();				
				if (visitaVacunaUO1 != null) arguments.putSerializable(Constants.VISITA, visitaVacunaUO1);
				...
				switch (opcion) {
					case Constants.CODIGO_TUBO_PBMC:
						...
						i.putExtra(Constants.ACCION, Constants.CODIGO_PROPOSITO_VC_UO1);
						break;
