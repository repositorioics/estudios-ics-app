...
public class NewPtActivity extends AbstractAsyncActivity {

	protected static final String TAG = NewPtActivity.class.getSimpleName();
	...
	private void createInitDialog() {
		...
		message.setText(getString(R.string.verify_pt));
	...
	protected void onActivityResult(int requestCode, int resultCode,
		...
		if (resultCode == RESULT_OK) {
			...
			if (complete.matches("complete")){
				...
				parseEstacionPT(idInstancia,instanceFilePath,cambio);
			}
			else{
	...
	public void parseEstacionPT(Integer idInstancia, String instanceFilePath, String ultimoCambio) {
		Serializer serializer = new Persister();
		File source = new File(instanceFilePath);
		try {
			PyTXml em = new PyTXml();
			em = serializer.read(PyTXml.class, source);
			
			PesoyTallaId ptId = new PesoyTallaId();
			ptId.setCodigo(codigo);
			ptId.setFechaPT(new Date());
			mPyT.setPtId(ptId);
			mPyT.setPeso1(em.getPeso1());
			mPyT.setPeso2(em.getPeso2());
			mPyT.setPeso3(em.getPeso3());
			mPyT.setTalla1(em.getTalla1());
			mPyT.setTalla2(em.getTalla2());
			mPyT.setTalla3(em.getTalla3());
			mPyT.setImc1(em.getImc1());
			mPyT.setImc2(em.getImc2());
			mPyT.setImc3(em.getImc3());
			mPyT.setDifPeso(em.getDifPeso());
			...
			mPyT.setEstudiosAct(mParticipante.getProcesos().getEstudio());
	...
	private void addPyT() {
		try{
			...
			Cursor c = getContentResolver().query(Constants.CONTENT_URI, projection,
					"jrFormId = 'estacionpesotalla' and displayName = 'Datos de Estacion Peso y Talla'", null, null);
			c.moveToFirst();
